//Double Connected Components(Points)
...
void search(int s){
    ...
    for (int i=0;i<f[s].size();i++){
        ...
        if (dfn[c]==0){
            ...
            if (low[c]>=dfn[s]){
                ++tot;
                while (true){two[tot].p_b(stack[top]);which[stack[top]]=tot;
...
// same(a,b) returns the index of the common component of a and b (-1 if in different components)
