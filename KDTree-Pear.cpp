...
int x[maxn],y[maxn],z[maxn],wei[maxn],xc[maxn],yc[maxn],zc[maxn],wc[maxn],hash[maxn],biao[maxn];
...
void makekdtree(int node,int l,int r,int flag)
{
    ...
    int xl=maxlongint,xr=-maxlongint,yl=maxlongint,yr=-maxlongint,zl=maxlongint,zr=-maxlongint,maxc=-maxlongint;
    for (int i=l;i<=r;i++)
    ...
    tree[node].flag=flag;
    tree[node].xl=xl,tree[node].xr=xr,tree[node].yl=yl,tree[node].yr=yr,tree[node].zl=zl,tree[node].zr=zr;
    tree[node].max=maxc;
    if (l==r) return;
...
int getmax(int node,int xl,int xr,int yl,int yr,int zl,int zr)
{
    ...
    if ((tree[node].xl==xl)&&(tree[node].xr==xr)&&
        ...
        (tree[node].zl==zl)&&(tree[node].zr==zr)) return tree[node].max; else
    return max(getmax(node*2,xl,xr,yl,yr,zl,zr),getmax(node*2+1,xl,xr,yl,yr,zl,zr));
}
